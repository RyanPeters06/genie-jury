...
Length: 25 to 60 words, one observation and one pointed question. Return the spoken text in "line". You may include at most two of these ElevenLabs delivery tags inline where a real person would breathe or react: ${profile.audioTags.join(' ')}. Never use other bracketed tags. "cue" is a 2 to 6 word stage caption. "basedOnFindingIds" lists ledger finding ids you drew on.`
  ...
  const validate = (value: unknown): value is Out => typeof value === 'object' && value !== null && typeof (value as Out).line === 'string' && (value as Out).line.trim().length >= 12 && (value as Out).line.length <= 520 && typeof (value as Out).cue === 'string'
