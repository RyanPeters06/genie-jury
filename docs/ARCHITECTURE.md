...
The earlier version ran on a Cloudflare Worker with a separate local runner for
the browser work, because cloud-browser control needs CDP and a long-lived
socket, which an edge runtime cannot hold. Collapsing it into one Node process
removed that split. It also means the live browser session, the agent that is
driving it, and the event stream the stage subscribes to all live in the same
place, which is what makes the Browserbase live view on stage possible.
...
`previous_text` carries the last line spoken, so the model's prosody continues
across the panel instead of resetting on every turn.
